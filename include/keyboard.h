...
typedef struct { uchar down,up; } keycodes;
typedef struct control {keycodes key; K_TRIGGER trigger; uint title_len; char title[];} control;
typedef struct controlscheme { uint count; control controls[]; } controlscheme;
...
//     //very neat scencodes for qwerty. see https://wiki.osdev.org/PS/2_Keyboard
